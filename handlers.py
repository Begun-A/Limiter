...
def count_timeout(domen, gen_time, rate_limit):
    ...
    if domen in DOMENS:  # check domen in DOMENS dict
        ...
        # Interval between now and last request time which display
        # how many time left befor last request(-)
        ...
        # #Uncomented code below if you want limit
        # crawlers timeout (> SECONDS-RATE_LIMIT*2).
        ...
        if gen_time >= rate_limit and (
            ...
            # Timeout equel generation time without
            # time_interval(+) time or with time_interval(-)
            timeout = gen_time - time_interval
            DOMENS[domen] = last_request + timedelta(seconds=gen_time)
        ...
        if rate_limit > gen_time and (
            ...
            # Timeout equal rate limit without
            # time_interval(+) or with time_interval(-)
            timeout = rate_limit - time_interval
            DOMENS[domen] = last_request + timedelta(seconds=rate_limit)
            # set last request time
            return timeout
